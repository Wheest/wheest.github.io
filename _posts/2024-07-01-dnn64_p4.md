...
Sure, the N64 hardware is old, but these are the same problems I've faced in others works using modern hardware, e.g., [the SECDA project](https://dl.acm.org/doi/10.1016/j.jpdc.2022.11.005) (led by the talented hardware systems designer Jude Haris).
Moreover, we're using modern software tooling, so the retro nature of the N64 is almost irrelevant, it's just in terms of resources it's now closer to a contemporary micro-controller.
...
Let's take a couple of excerpts that stood out to me, more on an aesthetic level than a technical one:
