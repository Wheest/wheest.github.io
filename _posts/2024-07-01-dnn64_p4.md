...
Its acceleration come from the vector unit, which is a MIPS core with vector instructions (128-bit lanes, with each of the 8 lanes being 16-bits wide).
...
One of the reasons why the project is appealing to me is that this high-level system design, of a CPU, with a co-processor with more limited memory, is very similar to modern DNN accelerator environments.
...
As discussed earlier, the RSP is programmed in assembly, and with a decent enough manual like this, is should be possible to get our accelerated DNN layers implemented this way.
