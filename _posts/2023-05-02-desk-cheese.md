...
The actual photo taking is managed by [fswebcam](https://www.sanslogic.co.uk/fswebcam/), a bash script, and a cronjob. For filtering the generated images, YOLOv3 is running on the CPU using [OpenCV's DNN backend](https://opencv.org/). This isn't the most performant, especially for someone whose PhD is ostensibly in accelerating deep learning workloads. However, given the model is not going to be running a lot, this is the most portable and lowest effort solution.
