---
layout: post
title:  "DNN64: A ML Compiler Toolchain for the Nintendo 64 (Part 1)"
...
<img src="/assets/pics/n64/headerp1.png" width="1024">
...
I endeavoured to use modern tools (e.g., Apache TVM), and acceleration techniques which are relevant to today's models and hardware accelerators, but applying them to the unique hardware and software environment of this beloved (by some, it was out before I was born) games console.
...
At the start of my PhD, I got more familiar with the Apache TVM tensor compiler, and it was what helped solidify my belief (and one of the key takeaways of my PhD thesis) that domain specific compilers will increasingly be the centre of the deep learning systems stack.
You can read a bit more about that stack in [my overview paper: "DLAS: An Exploration and Assessment of the Deep Learning Acceleration Stack"](https://arxiv.org/abs/2311.08909).

One thing I've wondered for a couple of years though is how easy it would be to get a modern DNN working on retro hardware, especially games consoles.
Some like to ape DNNs as the hot new thing, but they're [an old idea](https://en.wikipedia.org/wiki/Perceptrons_(book)), its just recently that the availability of large datasets and powerful hardware has made them more practical and scalable.
...
I'm going to structure this first post introducing the core components of my solution (the N64 toolchain, the N64 hardware, Apache TVM, and how we bring it all together), as well as the challenges we face regarding acceleration.  Future posts will discuss how does challenges are tackled, as well as efforts towards optimisation.
For the record, I have no experience with game console emulation and retro homebrew development.
So I came at these topics as an outsider.
...
We have two main options for our project: either we program in C and use an existing N64 compiler toolchain, or we get another toolchain (say TVM) to generate our in the N64's ISA directly.
...
Given that is was getting cumbersome, I looked further afield for a more specialised toolchain, ideally with an active commnity.
...
This is copyrighted, and not distributed by the authors for obvious reasons.
...
It makes it straight forward to include C standard library functions, and provides its own utilities for things such as 2D graphics programming (with a port of OpenGL for 3D graphics under development!), debugging, audio, and data management.
...
We can go into greater detail as required during this post series.
My major source for this information is this excellent summary by [https://www.copetti.org/writings/consoles/nintendo-64/](https://www.copetti.org/writings/consoles/nintendo-64/), as well as the [N64 brewwwiki](https://n64brew.dev/).
...
The system has a unified-memory architecture (UMA) of 4MB of RAM (extensible up to 8MB with an expansion pack).
This memory limit is going to be our main challenge for deploying DNNs: even modestly sizes DNNs can have dozens of MBs of parameters, not to mention their activations.
...
We often accelerate DNNs on GPUs, so is anything in the N64's Reality Co-Processor which could help us?
I think there is, a sub-component called the Reality Signal Processor (RSP), with the architecture shown in the figure below, courtesy of .
...
| ![rsp.png](/assets/pics/n64/rsp.png) |
|:--:|
| *Architecture of the Reality Signal Processor (RSP), figure taken from [Rodrigo Copetti](https://www.copetti.org/writings/consoles/nintendo-64/) under [CC BY 4.0 Deed](https://creativecommons.org/licenses/by/4.0/).* |

The core CPU of the N64 has a floating point unit (FPU), but we don't get floating point numbers in the RSP's vector unit.
...
| ![fixed_point.png](/assets/pics/n64/fixed_point.png) |
|:--:|
| *Example of a 32-bit fixed point number, with 16 bits for the integer part and 16 bits for the fractional part* |

Additionally, the RSP has a 4KB of instruction cache, and 4KB of data cache.
...
If you're interested in the topic of data movement in DNNs, I'd recommend the [AXI4MLIR paper](https://conf.researchr.org/details/cgo-2024/cgo-2024-main-conference/5/AXI4MLIR-User-Driven-Automatic-Host-Code-Generation-for-Custom-AXI-Based-Accelerator), which I co-authored.

When developing, I'm using an emulator, which is a software program that emulates the hardware of the N64.
...
I mentioned Apache TVM earlier, which is a tensor compiler which can take DNN models from a variety of sources (e.g., PyTorch, TensorFlow, ONNX), and generate efficient code for a range of platforms, including Arm, Intel, and RISC-V CPUs; Arm, AMD, and NVidia GPUs; OpenCL, CUDA, LLVM, Vulkan; and more.
I'll discuss more of how TVM works as required in this post series, but check out [it excellent docs and tutorials](https://tvm.apache.org/) if you're interested in learning more.
...
We could hardcode this for every model that we want to deploy, but that's not very compiler-brained.
Our programs should write themselves.
The approach I've gone for is to use the [Jinja templating engine](https://jinja.palletsprojects.com).
Typically used for generating HTML files, in frameworks such as [Flask](https://palletsprojects.com/p/flask/), I'm using it for C, defining a `main.c.template` file for our project, which we populate as required.
By using the double brace syntax `{{my_var}}`, we can define substitutions that can be placed in our code by the templating engine.
...
int main(void) {
  ...
  {{output_dtype}} expected_outputs[{{num_outputs_to_check}}];
  inputs.{{input_name}} = (void *)&{{sample_name0}};
  // rest of the code
}
```
...
| ![first_run.png](/assets/pics/n64/first_run.png) |
|:--:|
| *First run of a DNN on the N64.* |
...
We'll talk about those in future posts, as well as go deeper into my code, which I will be open sourcing.
For now, catch ye.
