...
I hope that this series of posts will help make clear the relevant components, how I designed and implemented them, and my strategies for optimisation.
...
Given that it was getting cumbersome, I looked further afield for a more specialised toolchain, ideally with an active community.
...
My adventure into finding a legal and functional boot code led me to the [libdragon](https://github.com/DragonMinded/libdragon) SDK, which includes open source boot code, as well as a lot of other quality-of-life tooling for N64 development.
...
The RSP is essentially another CPU package, with a scalar unit, which I believe is the manager of the RSP, and Vector Unit which does the bulk of the compute.
...
If we untar the file it generates, we should have all of the code we need to execute our model, including the model definition, weights, and runtime.
...
With our build system and main function ready, we can compile a small test model, and execute it on an N64 emulator.
Here it is!
